...
- **`shipping`**: Delivery cost calculation + multi-carrier shipping. `getShippingZones` / `calculateShipping` price delivery from the merchant's zones + distance tiers (always available). When the store has **Shippo** connected and you pass a destination `address_to` + `parcel` to `calculateShipping`, the response ALSO includes live carrier `rates[]` (USPS/UPS/FedEx/DHL — each with `rate_id`, `provider`, `service`, `amount`, `estimated_days`); show them at checkout and carry the chosen `rate_id` into `orders.createOrder` as `shippo_rate_id`. A `rate_source` field on the response tells you which source produced it (`zone`, `tier`, `shippo`, `custom`, or `none`) — if the merchant has connected their own carrier as a Custom Integration, its options appear in the same `rates[]` with `rate_source: 'custom'`, and you handle them exactly like a Shippo rate. For tracking, the lightest path is to link to the carrier's own tracking page (saved on the order at `shipping_metadata.shippo.tracking_url` when the label is bought — no API call); use `trackShipment({ carrier, number })` only when you want the live status rendered inside your own UI. (Buying a shipping label is a merchant fulfillment action done in the admin — it charges the merchant's carrier account, marks the order shipped, and emits `order.shipped` — so it isn't part of the storefront SDK.) Manual zones/tiers keep working with or without Shippo.
...
const unsubscribe = subscribeToThread({
  ...
  // Optional presence: who is connected to THIS conversation right now (per-thread,
  // not global). Use it for a "seller is online" indicator.
  onPresence: ({ type, participants }) => console.log(type, participants),
});
// later: unsubscribe();
```
...
```typescript
// Wholesale (B2B) — a wholesale store. Every call needs the buyer's session
...
// Self-serve reorder: place an order in one call. Send only items + quantities —
// the server prices each line from the buyer's wholesale price list.
const order = await client.b2B.createDirectOrder({
  xAuthToken: session,
  idempotencyKey: crypto.randomUUID(),
  requestBody: { items: [{ variant_id: "…", quantity: 10 }] },
});
// order.data.settlement is "terms" (invoice issued) or "pay_now" (pay against order_id).

// Negotiated order: request a quote → accept it into a purchase order → pay the invoice on terms.
